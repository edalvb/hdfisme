...
@Component({
  selector: 'app-mantenimiento-tarea',
  templateUrl: './mantenimiento-tarea.component.html',
  styleUrls: ['./mantenimiento-tarea.component.css']
})
export class MantenimientoTareaComponent implements OnInit {

  cfDgFTarea = new FormControl('', [Validators.required]);
  matcher = new MyErrorStateMatcher();

  models: any = []; // Este es un array que contienen a todos los registros que traiga de la base de datos.
  model: TareaMantenimiento = {
    idtarea_mantenimiento: null,
  ...
  ngOnInit() {
    this.leerTodosTarea(); // Se trae a todos los datos para mostrar en la tabla.
  }

  crear() {
    delete this.model.idtarea_mantenimiento;
    this.estadoService.crearTarea(this.model).subscribe(
      (res: any) => {
        this.leerTodosTarea();
  ...
  leerTodosTarea() {
    this.estadoService.leerTodosTarea().subscribe(
      (res: any) => {
  ...
  actualizar() {
    this.estadoService.actualizarTarea(this.model.idtarea_mantenimiento,
  ...
  eliminar(id: number) {
    this.estadoService.eliminarTarea(id).subscribe(
      (res: any) => {
        this.leerTodosTarea();
        this.mostrarMensaje(res.mensaje, 'Aceptar');
      },
