...
import { PrioridadMantenimiento } from "../../../models/Mantenimiento";
import { MantenimientoService } from 'src/app/services/mantenimiento.service';

@Component({
  selector: 'app-mantenimiento-prioridad',
  templateUrl: './mantenimiento-prioridad.component.html',
  styleUrls: ['./mantenimiento-prioridad.component.css']
})
export class MantenimientoPrioridadComponent implements OnInit {

  cfDgFPrioridad = new FormControl('', [Validators.required]);
  matcher = new MyErrorStateMatcher();

  models: any = []; // Este es un array que contienen a todos los registros que traiga de la base de datos.
  model: PrioridadMantenimiento = {
    idprioridad_mantenimiento: null,
    prioridad_mantenimiento: null,
    descripcion: null
  ...
  constructor(
    ...
    private prioridadService: MantenimientoService) { }

  ngOnInit(): void {
    this.leerTodosPrioridad(); // Se trae a todos los datos para mostrar en la tabla.
  }

  crear() {
    delete this.model.idprioridad_mantenimiento;
    this.prioridadService.crearPrioridad(this.model).subscribe(
      (res: any) => {
        this.leerTodosPrioridad();
  ...
  leerTodosPrioridad() {
    this.prioridadService.leerTodosPrioridad().subscribe(
      (res: any) => {
  ...
  actualizar() {
    this.prioridadService.actualizaPrioridad(this.model.idprioridad_mantenimiento,
  ...
  eliminar(id: number) {
    this.prioridadService.eliminarPrioridad(id).subscribe(
      (res: any) => {
        this.leerTodosPrioridad();
        this.mostrarMensaje(res.mensaje, 'Aceptar');
      },
